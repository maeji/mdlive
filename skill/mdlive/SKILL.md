---
name: mdlive
description: 마크다운(.md) 또는 HTML 파일을 macOS 네이티브 창으로 즉시, 그리고 실시간으로 미리보기한다. AI 에이전트(Claude Code, Codex 등)가 문서를 써 내려가는 동안 글이 채워지는 것을 라이브로 보여준다. 사용자가 "미리보기", "preview", "mdlive", "창으로 열어줘", "렌더링해서 보여줘", "실시간으로 보면서 고치자", "brief 보면서 다듬자"라고 하거나, 문서/리포트/리드미/brief를 만든 뒤 결과를 바로 눈으로 확인하고 싶을 때 사용한다.
---
...
작업 명세 `brief.md` 를 만들 때 특히 유용하다. `mdlive brief.md --watch` 로 띄워 두고
사용자와 함께 내용을 다듬으면, 고칠 때마다 창이 실시간으로 갱신된다. brief 가 충분히
구체화되면 `goal` skill 로 한 번에 발주한다.
